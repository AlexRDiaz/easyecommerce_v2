class ImagesSystem {
  String logoEasyEcommercce = "./assets/logo.png";
  String logoEasyEcommerceNoText = "./assets/logonotest.png";

  String logoFonles = "./assets/fonles-logo.png";
  String imageWelcome = "./assets/welcome.jpeg";
  String imageWelcomeLogo = "./assets/logoWhite.png";

  String menuIcon = "./assets/menu-icon.png";
  String whatsapp_icon = "./assets/whatsapp_icon.png";
  String whatsapp_icon_2 = "./assets/whatsapp_icon_2.png";
  String phone_call = "./assets/phone_call.png";

  String logoLogec = "./assets/logec_logo.png";
  String logoGtm = "./assets/gtm_logo.png";
  String logoLogec2 = "./assets/logec_logo2.png";
}
